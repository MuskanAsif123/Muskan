...
class favouritesViewModel : ViewModel() {
    ...
    fun getAllFavRecords(): LiveData<List<FavouritesRecord>> {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            for (i in 0..10){
                if(i%2==0)
                    records.add(FavouritesRecord(R.drawable.ic_baseline_favorite_24,"DSU Student-"+i,"Hello Description",isFav = true))
                else
                    records.add(FavouritesRecord(R.drawable.ic_baseline_favorite_24,"DSU Student-"+i,"Hello Description",isFav = false))
